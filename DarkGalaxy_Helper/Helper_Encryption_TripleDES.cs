...
namespace DarkGalaxy_Helper
{
    ...
    /// 提供一系列对于三重DES加密解密的操作
    /// </summary>
    public class Helper_Encryption_TripleDES : IEncryptionSymmetric
    {
        /// <summary>
        /// 使用三重DES与密钥加密原始字符串，返回加密后的字符串
        /// 加密失败则返回null
        /// </summary>
        ...
        public string Encryption(string originalString, out byte[] encryptionKey, Encoding encoding = null, CipherMode cipherModes = CipherMode.CBC, PaddingMode paddingModes = PaddingMode.None)
        {
            ...
            //进行三重DES加密
            TripleDESCryptoServiceProvider crypTripleDESCrypto = new TripleDESCryptoServiceProvider()
            {
                Mode = cipherModes,//设置运算模式
                Padding = paddingModes//设置填充模式
            };
            crypTripleDESCrypto.GenerateKey();
            encryptionKey = crypTripleDESCrypto.Key;
            MemoryStream strmMemory = new MemoryStream();
            using (CryptoStream strmCrypto = new CryptoStream(strmMemory, crypTripleDESCrypto.CreateEncryptor(), CryptoStreamMode.Write))
            {
        ...
        public string Decryption(string originalString, byte[] encryptionKey, Encoding encoding = null, CipherMode cipherModes = CipherMode.CBC, PaddingMode paddingModes = PaddingMode.None)
        {
            ...
            MemoryStream strmMemory = new MemoryStream();
            using (CryptoStream strmCrypto = new CryptoStream(strmMemory, crypTripleDESCrypto.CreateDecryptor(), CryptoStreamMode.Write))
            {
                strmCrypto.Write(arrData, 0, arrData.Length);
                strmCrypto.FlushFinalBlock();
        ...
        /// <summary>
        /// 使用三重DES与密钥和向量加密原始字符串，返回加密后的字符串
        /// 加密失败则返回null
        /// </summary>
        /// <param name="originalString">原始字符串</param>
        /// <param name="encryptionKey">密钥</param>
        ...
        public string Encryption(string originalString, out byte[] encryptionKey, out byte[] encryptionIVector, Encoding encoding = null, CipherMode cipherModes = CipherMode.CBC, PaddingMode paddingModes = PaddingMode.None)
        {
            ...
            crypTripleDESCrypto.GenerateKey();
            crypTripleDESCrypto.GenerateIV();
            encryptionKey = crypTripleDESCrypto.Key;
            encryptionIVector = crypTripleDESCrypto.IV;
            MemoryStream strmMemory = new MemoryStream();
            using (CryptoStream strmCrypto = new CryptoStream(strmMemory, crypTripleDESCrypto.CreateEncryptor(), CryptoStreamMode.Write))
            {
                strmCrypto.Write(arrData, 0, arrData.Length);
        ...
        /// <summary>
        /// 使用三重DES与密钥和向量解密原始字符串，返回解密后的字符串
        /// 解密失败则返回null
        /// </summary>
        /// <param name="originalString">原始字符串</param>
        /// <param name="encryptionKey">密钥</param>
